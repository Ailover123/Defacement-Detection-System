...
from crawler.storage.db import (
    ...
    fail_crawl_job,
    has_site_crawl_data,
    get_support_defacement_email,
    get_data_by_fld_name,
    get_custname_data
)
...
mail_from = "contact@pagentra.com"
mail_server = "ws.sitewall.net:25"
home = os.getenv("app.publicPath")

primary_url = ""
admin_url = ""
admin_url_data = get_data_by_fld_name("admin_url")
if admin_url_data:
    admin_url = admin_url_data[0]['file_path']
    logger.error(f"Admin url found :- {admin_url}")
else:
    logger.error(f"Admin url not found.")
    sys.exit()

if home == '/home/devadminsitewall':
    primary_url = admin_url
else:
    url = get_data_by_fld_name("scanner_url")
    primary_url = url
...
def crawl_site(site, args, target_urls=None):
    ...
    try:
        ...
        if CRAWL_MODE == "BASELINE":
            ...
            logger.info(f"Targetted URL {target_urls}.")
            ...
            job_logger.info(f"Baselines content_hash  : {stats.get('content_hash', 0)}")
            job_logger.info("-" * 60)
            return stats
        ...
        if CRAWL_MODE == "CRAWL":
            ...
            job_logger.info(f"New urls found: {all_new_urls}")
            
            mail_to = get_support_defacement_email()

            if mail_to:
                job_logger.info(f"Send mail to : {mail_to}")
            else:
                mail_to = "support@pagentra.com"
            ...
            if all_new_urls:
                ...
                response = requests.post(
                    primary_url+"/SentralHub/insert-defacement",
                    json=all_new_urls,
                    timeout=60
                )
                job_logger.info(f"API all_new_urls: {all_new_urls[0]}")
                job_logger.info(f"API Response: {response.status_code} - {response.text}")
                
                if response.status_code == 200:
                    site = all_new_urls[0]['base_url']
                    custname = get_custname_data('932')
                    job_logger.info(f"site name :- {site}, customer name :- {custname}")
                    subj = f"Defacement - New Links Found For: {site} for {custname}"
                    job_logger.info(f"Sending Mail to Support Team for {site} for {custname}") 
                    msg = f"""
                        <!DOCTYPE html>  
                        <html>
                        <head>
                        ...
                        <p style=\"font-size:11.0pt\">Hi Administrator,</p>
                        <p style=\"font-size:11.0pt\">New Links has been found for <a href=\"javascript:void(0)\" style=\"text-decoration: none;\">{site}</a>. For more details please login and go to the link given below</p>
                        <p style=\"font-size:11.0pt\"><a href=\"{admin_url}/sentral/deflist\" target=\"_blank\">Admin Sentral</a></p>
                        <p style=\"font-size:11.0pt\">Warmest Regards,<br>
                        SiteWALL Team<br><br> 
                        </div>
                        </body>
                        </html>
                    """
                    email = f"/home/devadminsitewall/defacement/sendEmail.pl -f '{mail_from}' -t '{mail_to}' -u '{subj}' -s {mail_server} -o tls=no reply-to={mail_from} message-content-type=html -m '{msg}'"
                    email = shlex.split(email)
                    process = subprocess.Popen(email, stdout=subprocess.PIPE)
                    (stdout, stderr) = process.communicate()
                    job_logger.info(f"Sent to --> {mail_to} {stdout.decode().strip()}")

                # except Exception as e:
                #     job_logger.error(f"API Error: {str(e)}")
...
def main():
    ...
    parser.add_argument("--stat",type=str,choices=["ADD"],help="Add first time baseline")
    ...
    if args.baseline_id:
        if args.baseline_id and args.stat:
            sites = [{
                "siteid": "93245", 
                "custid": "932", 
                "url": "ugaoo.com"
            }]
            logger.info(f"--> Targeting specific Baseline ID: {args.baseline_id} ")
        else:
